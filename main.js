...
$packages["./main"] = (function() {
	var $pkg = {}, $init, js, runtime, ptrType, funcType, funcType$1, npm, app, process, tray, win, main, createTray, openWindow;
	js = $packages["github.com/gopherjs/gopherjs/js"];
	...
	main = function() {
		...
		app.on($externalize("ready", $String), $externalize((function() {
			process.on($externalize("uncaughtException", $String), $pkg.Alert.uncaughtException($externalize(false, $Bool), $externalize((function(err) {
				var err;
				app.exit(1);
			}), funcType)));
			createTray();
		}), funcType$1));
		app.on($externalize("window-all-closed", $String), $externalize((function() {
		}), funcType$1));
	};
	createTray = function() {
		tray = new ($pkg.Tray)($externalize("./assets/tray.png", $String));
		tray.on($externalize("click", $String), $externalize((function() {
			openWindow();
		}), funcType$1));
	};
	openWindow = function() {
		var opts;
		if (win === null) {
			opts = $makeMap($String.keyFor, [{ k: "width", v: new $Int(800) }, { k: "height", v: new $Int(600) }, { k: "title", v: new $String("Visual Markdown") }, { k: "show", v: new $Bool(false) }, { k: "skipTaskbar", v: new $Bool(true) }, { k: "minimizable", v: new $Bool(false) }, { k: "webPreferences", v: new js.M($makeMap($String.keyFor, [{ k: "nodeIntegration", v: new $Bool(true) }, { k: "devTools", v: new $Bool(false) }])) }]);
			win = new ($pkg.BrowserWindow)($externalize(opts, js.M));
			win.removeMenu();
			win.loadFile($externalize("renderer.html", $String));
			win.once($externalize("ready-to-show", $String), $externalize((function() {
				win.show();
			}), funcType$1));
			win.on($externalize("close", $String), $externalize((function(e) {
				var e;
				e.preventDefault();
				win.hide();
			}), funcType));
	...
	$init = function() {
		...
		$pkg.Tray = npm.Tray;
		app = npm.app;
		$pkg.BrowserWindow = npm.BrowserWindow;
		$pkg.Alert = npm.Alert;
		process = npm.process;
		if ($pkg === $mainPkg) {
			main();
